...
void GameScreen::allocateEnemy()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        enemy[i] = new DynamicEntity("smallufo.txt", {SHORT(random + (hero->getHeroWidth() + 40) * i + 13 * (i - 1)), SHORT(3)}, {11, 5});
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        enemy[i + 9] = new DynamicEntity("smallufo.txt", {SHORT(random + (hero->getHeroWidth() + 40) * i + 13 * (i - 1)), SHORT(23)}, {11, 5});
    }
    random = rand() % (appConsole.getWindowSize().X / 2 - 25);
    for (int i = 0; i < 4; ++i)
    {
...
void LeaderBoardScreen:: draw(){
    ...
    if (firstScreen)
    {
        ...
        for(int i = 2;i>=0;i--){
        importImage.drawCustomImage(data[i].name, {47, SHORT(18+i*7)},false);
        importImage.drawCustomImage(data[i].score, {82, SHORT(18+i*7)},false);  
